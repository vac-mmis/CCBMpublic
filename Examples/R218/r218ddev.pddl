(define (domain r218ddev)
  ...
  (:predicates
   ...
   (is-wired ?d - device ?p - port ?cp - cb-port)
   (is-open ?cp - cb-out-port)
   (is-switched ?cpi - cb-in-port ?cpo - cb-out-port)
   )
...
;; Crossbar
  ...
  (:action xxx-device-crossbar-switch
	   :parameters (?ip - cb-in-port ?op - cb-out-port)
	   :precondition (is-open ?op)
	   :effect (and (not (is-open ?op))
			(is-switched ?ip ?op)))

  (:action xxx-device-crossbar-unswitch
	   :parameters (?ip - cb-in-port ?op - cb-out-port)
	   :precondition (is-switched ?ip ?op)
	   :effect (and (is-open ?op)
			(not (is-switched ?ip ?op))))
