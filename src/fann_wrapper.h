...
public:
    ...
    */
    bool create_standard(unsigned int num_layers, ...) {
        unsigned int *data = new unsigned int[num_layers];

        va_list layers;
        va_start(layers, num_layers);
        for (unsigned int i=0; i<num_layers; i++)
            data[i] = va_arg(layers, unsigned int);
        va_end(layers);

        bool status = create_standard_array(num_layers, data);
        delete[] data;
        return status;
    }
    ...
    */
    bool create_sparse(float connection_rate, unsigned int num_layers, ...) {
        ...
        bool status = create_sparse_array(connection_rate, num_layers, data);
        delete[] data;
        return status;
    }
    ...
    */
    bool create_shortcut(unsigned int num_layers, ...) {
        ...
        bool status = create_shortcut_array(num_layers, data);
        delete[] data;
        return status;
    }
